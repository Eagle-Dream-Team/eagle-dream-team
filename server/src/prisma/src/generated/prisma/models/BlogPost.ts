...
export interface BlogPostDelegate<ExtArgs extends runtime.Types.Extensions.InternalArgs = runtime.Types.Extensions.DefaultArgs, GlobalOmitOptions = {}> {
   ...
   * const blogPostWithPost_idOnly = await prisma.blogPost.findMany({ select: { post_id: true } })
   * 
   */
   ...
   * })
   * 
   * // Create many BlogPosts and only return the `post_id`
   * const blogPostWithPost_idOnly = await prisma.blogPost.createManyAndReturn({
   *   select: { post_id: true },
   *   data: [
   *     // ... provide data here
   *   ]
   ...
   * })
   * 
   * // Update zero or more BlogPosts and only return the `post_id`
   * const blogPostWithPost_idOnly = await prisma.blogPost.updateManyAndReturn({
   *   select: { post_id: true },
   *   where: {
   *     // ... provide filter here
   *   },
